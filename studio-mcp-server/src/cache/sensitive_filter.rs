...
impl SensitiveDataFilter {
    ...
    pub fn should_skip_caching(&self, key: &str) -> bool {
        ...
        let sensitive_key_patterns = [
            "auth", "token", "secret", "password", "credential", "login",
            "session", "jwt", "oauth", "api_key", "private_key", "cert",
            "certificate", "keystore", "vault", "encryption"
        ];

        let key_lower = key.to_lowercase();
        for pattern in &sensitive_key_patterns {
            if key_lower.contains(pattern) {
                debug!("Skipping cache for sensitive key pattern '{}': {}", pattern, key);
                return true;
            }
        }
    ...
    pub fn filter_value(&self, value: &Value) -> Value {
        match value {
            ...
            Value::Array(arr) => {
                Value::Array(arr.iter().map(|v| self.filter_value(v)).collect())
            }
            Value::String(s) => {
                self.filter_string_value(s)
            }
            _ => value.clone(),
        }
    }
            ...
            if pattern.regex.is_match(value) {
                match pattern.severity {
                    Severity::Critical | Severity::High => {
                        ...
                        was_filtered = true;
                        warn!("Filtered {} pattern '{}' from cache value", 
                              match pattern.severity {
                    ...
                    Severity::Medium => {
                        ...
                        debug!("Detected medium sensitivity pattern '{}' in cache value", pattern.name);
                    }
                }
            }
        }
    ...
    fn build_patterns() -> Vec<CompiledPattern> {
        ...
        if let Ok(regex) = Regex::new(r#"(?i)(api[_-]?key|apikey)[=:\s]+['"]?([a-zA-Z0-9_-]{20,})['"]?"#) {
            patterns.push(CompiledPattern {
                regex,
        ...
        // Bearer tokens
        if let Ok(regex) = Regex::new(r"(?i)bearer\s+[a-zA-Z0-9_-]{20,}") {
            patterns.push(CompiledPattern {
                regex,
        ...
        if let Ok(regex) = Regex::new(r#"/etc/[a-zA-Z0-9_/-]+|/var/[a-zA-Z0-9_/-]+|C:\\[a-zA-Z0-9_\\-]+"#) {
            patterns.push(CompiledPattern {
                regex,
    ...
    fn build_sensitive_fields() -> HashSet<String> {
        [
            ...
            "access_token", "refresh_token", "api_key", "apikey", "auth_key",
            "private_key", "public_key", "key", "certificate", "cert",
            
            // Session and auth
            "session", "session_id", "session_token", "csrf_token",
            "authorization", "credentials", "credential",
            
            // Encryption and security
            "encryption_key", "decrypt_key", "salt", "hash", "signature",
            "keystore", "truststore", "vault_token",
            
            // Database and connection
            "connection_string", "database_url", "db_password", "db_user",
            "jdbc_url", "redis_password", "mongo_uri",
            
            // Personal information
            "ssn", "social_security", "credit_card", "bank_account",
            "email_password", "phone", "address",
            
            // Cloud provider secrets
            "aws_access_key", "aws_secret_key", "azure_key", "gcp_key",
            "service_account_key", "client_secret", "client_id",
        ]
        .iter()
        .map(|s| s.to_string())
        .collect()
    ...
    fn build_sensitive_keywords() -> HashSet<String> {
        [
            ...
            "private", "confidential", "sensitive", "encrypted", "secure",
            "cert", "signature", "hash", "vault", "keystore",
        ]
        .iter()
        .map(|s| s.to_string())
        .collect()
