...
use studio_mcp_shared::{StudioConfig, StudioConnection, CliConfig, CacheConfig, LoggingConfig};
    ...
    let config = match StudioConfig::load_or_default(config_path) {
        Ok(config) => {
            ...
            if config.connections.is_empty() {
                error!("No connections configured. Run with --init to create a default configuration.");
                eprintln!("Error: No connections configured.");
                eprintln!("Run `{} --init` to create a default configuration.", args[0]);
                std::process::exit(1);
            }
...
async fn init_config(args: &[String]) -> anyhow::Result<()> {
    ...
    connections.insert("mock_studio".to_string(), StudioConnection {
    ...
    match config.save(config_path) {
        Ok(_) => {
            println!("✅ Configuration file '{}' created successfully!", config_path);
            println!();
